<template>
  <div>
    ...
    <div class="good-pay-bottombar">
      <div class="sum-price">
        总计:
        <span style="color:#ed7a5d">￥{{totalAmount}}</span>
      </div>
    ...
    <view-box>
      <div style="margin-top: 45px;" class="good-pay-address" @click="handleSetAddress('address')">
        <address-card v-if="selectedAddress" :link="true" :tool="false" :data="selectedAddress"></address-card>
        <div v-else class="gp-address-empty">
          请选择收货地址
          <span class="zui-icon zui-icon-arrow"></span>
        </div>
      </div>
      <div class="shop-card-body">
        <good-list v-for="(item, index) in payList" :key="index" :type="'INCONFIRM'" :data="item"></good-list>
      </div>
      <div v-show="payList.length!=0" class="shop-card-foot">
    ...
    <popup v-model="showPopup" height="400px" @on-first-show="resetScroller">
      <pay-popup :type="popupType" :popupData="popupData"></pay-popup>
    </popup>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    payType() {
      return this.$store.state.cart.payType;
    }
  },
  methods: {
      ...
      if (token !== null) {
        createOrder(token, products, amount, addressIndex, null, data => {
          if (!data.errcode) {
            ...
            let payType = this.$store.state.cart.payType;
            if (payType === "fromCart") {
